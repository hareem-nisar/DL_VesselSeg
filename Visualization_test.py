...
with torch.no_grad():
    for i, test_data in enumerate(test_loader):
        ...
        test_output = sliding_window_inference(
            test_data, roi_size, sw_batch_size, model)
        
        # plot the slice [:, :, 80]
        plt.figure("check", (20, 4))
        plt.subplot(1, 5, 1)
        plt.title(f"image {i}")
        plt.imshow(test_data.detach().cpu()[0, 0, :, :], cmap="gray")
        plt.subplot(1, 5, 2)
        plt.title(f"argmax {i}")
        argmax = [AsDiscrete(threshold_values=True)(i) for i in decollate_batch(test_output)]
